...
private:
    void log_kp_and_torques(const std::vector<float>& kp, const std::string& filename) {
        ...
        // Log torque values
        logfile << "Torques: ";
        for (const auto& value : torque_est) {
            logfile << value << ",";
        }
        logfile << "\n";
    ...
    void log_foot_forces( const std::string& filename) {
        std::ofstream logfile;
        logfile.open(filename, std::ios_base::app); // Open in append mode
        ...
        // Log kp values
        logfile << "foot force: ";
        for (const auto& value : foot_force_) {
            logfile << value << ",";
        }
        logfile << "\n";
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        modelPath = modelPath / "python" / "torchscript_model" / "VIC_2_model_HW.pt";
        // std::filesystem::path modelPath = "/home/dspoljaric/Desktop/unitree_mujoco/example/python/torchscript_model/VIC_2_model_HW.pt";
