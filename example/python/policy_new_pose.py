...
class PolicyVicClass:
    def __init__(self, cfg):
        ...
        self.d_gains = torch.ones(12) * cfg.env.control.d_gain
        ...
        self.load_model()
        print(f"Action scale: {cfg.env.action_scale}")
        
        self.action_scale = cfg.env.action_scale
        self.hip_scale = cfg.env.hip_scale
    ...
    def load_model(self):
        ...
        output_model_path = os.path.join(os.getcwd(), "new_pose.pt")
    ...
    def HighStateHandler(self, msg: SportModeState_):
        ...
        local_vel = self.rotate_vector(self.quat_invert(self.quaternion), glob__lin_vel) 
    ...
    def on_press(self, key):
        ...
        try:
            if key == keyboard.Key.up:
                ...
                print(f"[S]tart walking")
                self.policy_id[0] = 0
            elif key.char == 'n':
                print(f"[N]ull command")
                self.command[:] = 0.0
            elif key.char == 'q':
                stop_loop = True
                return False
    ...
    def test(self, config: DictConfig):
        ...
        while True:
            ...
            if self.policy_id[0] == 0:
                ...
                last_action = actions                
                scaled_action = actions * self.action_scale
                scaled_action[0] = scaled_action[0] * self.hip_scale
                scaled_action[3] = scaled_action[3] * self.hip_scale
                scaled_action[6] = scaled_action[6] * self.hip_scale
                scaled_action[9] = scaled_action[9] * self.hip_scale
                target_dof_pos = scaled_action[:12] + self.default_pos[7:]
            ...
            while total_fast_time < self.dt_slow:
                ...
                if self.runing_time_slow < 3.0 and self.policy_id[0] == 0:
                    ...
                    for i in range(12):
                        ...
                        cmd.motor_cmd[i].kp = phase * Kp[i] + (1 - phase) * 20.0
                        cmd.motor_cmd[i].dq = 0.0
                    ...
                    if self.control_mode == "P":
                        for i in range(12):
                            # print(f"target pos: {target_dof_pos[i]}")
                            # print(f"Kp: {Kp[i]} and Kd: {Kd[i]}")
                            cmd.motor_cmd[i].q = target_dof_pos[i]
                            cmd.motor_cmd[i].kp = Kp[i]
                            cmd.motor_cmd[i].dq = 0.0
                            cmd.motor_cmd[i].kd = Kd[i]
