...
@hydra.main(config_path='config', config_name='test_vic', version_base="1.2")
def test(config: DictConfig):
    ...
    last_action = torch.zeros(num_actions).to(device)
    obs[-3:]=command*2.0
    ...
    while True:
        ...
        if policy_id[0]==0:
            m = (stiff_range[0] +stiff_range[1])/2
            r= (stiff_range[1]-stiff_range[0])/2
            actions = actor_critic.forward(obs)
            last_action = actions
            target_dof_pos = actions[:12]*0.3 + default_pos[7:]
            if cfg["control_mode"] == "VIC_1":
                action_stiffness = torch.tile(actions[12:],4)
                Kp = p_gains*( m + r* action_stiffness)
            if cfg["control_mode"] == "VIC_2":
                action_stiffness = torch.tile(actions[12:],3)
                Kp = p_gains*( m + r* action_stiffness)
            if cfg["control_mode"] == "VIC_3":
                Kp = p_gains*( m + r* actions[12:])
...
if __name__ == '__main__':
    ...
    cfg ={"control_mode": "VIC_2", "actor_hidden_dim": 256,"actor_n_layers": 6,"critic_hidden_dim": 256,"critic_n_layers": 6,"std": 1.0}
    num_single_obs=48
    num_single_priv_obs = 59
    num_actions = 12
    if cfg["control_mode"] == "VIC_1":
        num_actions += 3
        num_single_obs += 3
        num_single_priv_obs += 3
    elif cfg["control_mode"] == "VIC_2":
        num_actions += 4
        num_single_obs += 4
        num_single_priv_obs += 4

    elif cfg["control_mode"] == "VIC_3":
        num_actions += 12
        num_single_obs += 12
        num_single_priv_obs += 12
    else:
        assert False, f"Control mode {cfg['control_mode']} not supported"
    ...
    p_gains = torch.ones(12) * 50


    ckpt_path = os.path.join(os.getcwd(),"best_models/test_vic_2.pt") #trained_terrain_l2
